class Friendly {
  ...
  void collision() {
    for (int i = 0; i < friendlies.length; i++) {
      ...
      if ((player.xpos + spaceship_width) > friendlies[i].xpos && player.xpos < (friendlies[i].xpos + spaceship_width) && (player.ypos + spaceship_height) > friendlies[i].ypos && player.ypos < (friendlies[i].ypos + spaceship_height)) 
      {
